...
4. Recommended for character-like feel:
   - Optional: set `ExistingWalkCharacter` to a placed level character (manager will possess this on Tab).
   - Optional auto-detect: enable `bAutoFindExistingWalkCharacter` and use `ExistingWalkCharacterTag`.
...
- **Walk mode toggles but WASD does not move**
  - If you already placed a character in-level, assign `ExistingWalkCharacter` (or use `bAutoFindExistingWalkCharacter` + tag).
