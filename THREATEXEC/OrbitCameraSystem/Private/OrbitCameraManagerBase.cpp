...
void AOrbitCameraManagerBase::EnterWalkOutMode()
{
	...
	if (PlayerController)
	{
		ActiveWalkCharacter = ResolveWalkCharacterTarget();
		if (ActiveWalkCharacter)
		{
			ActiveWalkCharacter->SetActorLocationAndRotation(GetActorLocation(), GetActorRotation());
			if (AOrbitWalkCharacter* OrbitWalkCharacter = Cast<AOrbitWalkCharacter>(ActiveWalkCharacter))
			{
				OrbitWalkCharacter->InitializeFromManager(
					this,
					WalkMoveSpeed,
					...
					MoveDownKey,
					SprintKey);
				WalkCharacterInstance = OrbitWalkCharacter;
			}
			PlayerController->Possess(ActiveWalkCharacter);
			PlayerController->SetViewTarget(ActiveWalkCharacter);
		}
	}
...
void AOrbitCameraManagerBase::ExitWalkOutMode()
{
	...
	if (ActiveWalkCharacter)
	{
		SetActorLocationAndRotation(ActiveWalkCharacter->GetActorLocation(), ActiveWalkCharacter->GetActorRotation());
		...
		if (bDestroyWalkCharacterOnExit && bWalkCharacterSpawnedByManager)
		{
			ActiveWalkCharacter->Destroy();
			WalkCharacterInstance = nullptr;
		}
	}
	ActiveWalkCharacter = nullptr;
...
void AOrbitCameraManagerBase::ApplyDesiredViewTarget()
{
	...
	if (bIsWalkOutMode)
	{
		DesiredViewTarget = (ActiveWalkCharacter)
			? Cast<AActor>(ActiveWalkCharacter.Get())
			: Cast<AActor>(this);
	}
	else
	{
...
ACharacter* AOrbitCameraManagerBase::ResolveWalkCharacterTarget()
{
	if (ExistingWalkCharacter)
	{
		bWalkCharacterSpawnedByManager = false;
		return ExistingWalkCharacter;
	}

	if (bAutoFindExistingWalkCharacter)
	{
		TArray<AActor*> FoundCharacters;
		UGameplayStatics::GetAllActorsOfClass(this, ACharacter::StaticClass(), FoundCharacters);
		for (AActor* FoundActor : FoundCharacters)
		{
			...
			if (!ExistingWalkCharacterTag.IsNone() && !FoundCharacter->ActorHasTag(ExistingWalkCharacterTag))
			{
				continue;
			}
	...
	if (!WalkCharacterInstance && WalkCharacterClass)
	{
		...
		{
			bWalkCharacterSpawnedByManager = true;
		}
	}

	return WalkCharacterInstance;
}
...
void AOrbitCameraManagerBase::UpdateDebugOverlay(float DeltaTime)
{
	...
	const FString WalkCharName = ActiveWalkCharacter ? ActiveWalkCharacter->GetName() : TEXT("None");
	...
	if (bDebugWalkStateEnabled)
	{
		...
		if (ActiveWalkCharacter)
		{
			GEngine->AddOnScreenDebugMessage(1006, 0.0f, FColor::Green, FString::Printf(TEXT("[Orbit Debug] WalkLoc: %s"), *ActiveWalkCharacter->GetActorLocation().ToCompactString()));
		}
	}
