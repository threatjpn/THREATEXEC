...
#pragma region OrbitAndWalkMode
	...
	// Optional existing character in the level to possess when entering walk mode.
	UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category = "OrbitCamera|WalkOut")
	TObjectPtr<ACharacter> ExistingWalkCharacter = nullptr;

	// If true and ExistingWalkCharacter is not set, manager auto-finds a placed character to use for walk mode.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "OrbitCamera|WalkOut")
	bool bAutoFindExistingWalkCharacter = true;

	// Optional tag filter used when auto-finding existing walk characters.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "OrbitCamera|WalkOut",
		meta = (EditCondition = "bAutoFindExistingWalkCharacter"))
	FName ExistingWalkCharacterTag = NAME_None;
...
private:
	...
	ACharacter* ResolveWalkCharacterTarget();
	...
	TObjectPtr<ACharacter> ActiveWalkCharacter = nullptr;
	bool bWalkCharacterSpawnedByManager = false;
